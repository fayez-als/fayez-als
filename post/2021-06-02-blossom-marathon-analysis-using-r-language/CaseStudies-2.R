...
df2001 <- select(df2001,c('place','ag','name','hometown','time'))
df2001$year <- 2001
...
colnames(Df) <- c('Place','Age','Name','Hometown','Time','Year','Gender')
...
Df %>% ggplot(aes(x=Year,y=Age)) + geom_boxplot()
...
Df$timesplit <- str_split(Df$Time,':')


Df$timesplit

Df$timesplitint <- sapply(Df$timesplit,as.numeric)


lengths(Df$timesplitint[5])
...
Df$minutes <- sapply(Df$timesplitint,toMinutes)
...
write.csv(Df,"Blossom2.csv", row.names = FALSE)



Df <- select(Df,c('Place','Age','Name','Hometown','Time','Year','minutes'))
